 ...
 let westrabotlistyetkilisi = db.fetch(`westrabotlistyetkilirol_${message.guild.id}`)
  ...
  message.delete()
   const westrabumbeyyyy = new Discord.MessageEmbed()
    .setColor(`GREEN`)
    .setFooter(`Kayez BotList Sistemi`)
    .setTimestamp()
    .setDescription(`<:mavitick:931993701834711150>  <@${sahip}> adlı kişinin <@${botisim}> adlı botu onaylandı. \nOnaylayan yetkili: ${message.author}`)
		client.channels.cache.get(log).send(westrabumbeyyyy);
		message.channel.send(`<:mavitick:931993701834711150>  Botu onayladınız.`).then(x => x.delete({timeout: 3000}))
};
