...
client.on("guildMemberAdd", member => {
  ...
  if (ayyy > 1) {
    kontrol = "**Güvenilir** <:mavitick:931993701834711150> ";
  }
...
//-------------------- Mod Log Sistemi --------------------//
 ...
 //
  .setDescription(`<a:zil:935232169993981963> **Hoş geldin!** ${
        member.user
      }, seninle beraber **${
        guild.memberCount
...
client.on("message", msg => {
    ...
    .setThumbnail(msg.author.displayAvatarURL({dynamic : true}))
    .setColor('#f6ff00')
    .setTitle("▬▬▬▬[<a:sasa:778787940741677088><a:sasa:778787940741677088> Yardım Mesajım ]▬▬▬▬\n ")
...
client.on('guildMemberAdd', member => {
 ...
 member.roles.add(member.guild.roles.cache.get(rol))
    ...
    .setFooter('Kayezi Tercih Ettiğiniz İçin Teşekkür Ederiz.')
...
client.on("message", (message) => {
  ...
  if (message.content.toLowerCase().startsWith(prefix + "roliste")) {
    ...
    let firstOption = new disbut.MessageMenuOption()
      .setValue("first")
      .setDescription("Kayez Bildirim Rolünü Alırsınız")
      .setLabel("Kayez Bildirim");
